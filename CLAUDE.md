...
- Domain glossary: `domain/corpus.json` — the course's own vocabulary, linked into the prose at runtime. See `../shared/domain/README.md` (same sibling repo as the walkthrough runtime). Most entries were harvested from the `.term-tooltip` definitions already in `output/*.html`, so the wording is the course's own; the runtime never touches an existing tooltip. Integration is one `<!-- DG:BUNDLE -->` before `</body>` — there is no per-module mount and no page code to change. Rebuild with `python ../shared/domain/build.py --corpus domain/corpus.json --root . --target "output/*.html" --prose "output/*.html" --prose-mode html` (idempotent). A `watchlist` word used in the prose that resolves to no term **fails the build** — the vocabulary counterpart of the Honesty Rule. `--report` lists what is still undefined.

- Worked examples: `domain/figures.json` — one per module page, the module's idea happening to the orderflow sample project, inserted after the objectives section by `python domain/insert_figures.py` (idempotent; it replaces rather than stacks). Vocabulary and styling come from `../shared/figure` — inline the stylesheet with `python ../shared/figure/build.py --target "output/*.html"`. The class vocabulary is closed: `python ../shared/figure/check.py --figures domain/figures.json` fails on anything outside it, on inline `style=`, on a hardcoded hex, or on a figure that links out. Every symbol, file and count in a figure must be real — `labs/sample-project` and `labs/M01-graph-fundamentals/solution/graph.json` are the sources; token counts are illustrative and say so.

## Key Design Rules (always apply)
1. Every technical term goes in **`domain/corpus.json`**, not in a hand-written tooltip. The runtime links its first use in *every* page, so a definition is no longer trapped in whichever module happened to carry it — `OKF` is why: 230 uses across the course, once expanded in M05, while M10 used it 55 times and defined it nowhere. The 52 `.term-tooltip` spans that predated the corpus were retired by `domain/retire_tooltips.py`; their wording lives on in the corpus, because that is where most of it was harvested from. A hand-written tooltip is still legal and still wins — the runtime skips `.term-tooltip` — but only for a gloss that must differ *in this module* from the canonical one. If it does not differ, it will drift; six words were defined two different ways before this.
2. Every concept gets: analogy → technical definition → animated visual → "why it matters" — and every module opens with a **worked example** showing the idea happening to real code, with the failure and the fix side by side. Show the artefact; do not assert that the reader can picture it.
