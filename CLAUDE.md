...
1. Every technical term gets a tooltip definition on FIRST use — and every term the course leans on goes in `domain/corpus.json`, which reaches the reader in **every** module rather than only the one that happened to define it. `OKF` is why: 230 uses across the course, expanded in a tooltip in M05 alone, while M10 uses it 55 times and defined it nowhere.
