"""M02B starter (fill in the TODOs) — fix the poisoned transcript with the retrieve and offload levers.
...
class ContextBudget:
    ...
    def fix_by_retrieve(self) -> dict:
        """TODO 1: the retrieve lever.
        1. deep-copy self.t
        2. answer = m09.explore("decode_jwt"); raise on "error"
        3. add answer["transitive_callers"]: BFS over m09._reverse from
           m09._resolve("decode_jwt") node ids (the M01 walk), short names
        4. replace t["tool_results"] with ONE entry containing the answer JSON
        """
        raise NotImplementedError("TODO 1")

    # -- arm 4: offload (this course's lever: one concept read) --------------
    def fix_by_offload(self) -> dict:
        """TODO 2: the offload lever.
        1. deep-copy self.t
        2. load WAU_CONCEPT with frontmatter.load; build one text block from
           its title + body
        3. replace t["retrieved"] (both stale chunks) with that ONE block
        4. drop every history turn that mentions WAU (settled knowledge now)
        """
        raise NotImplementedError("TODO 2")

    # -- arm 5: combined (retrieve + offload) --------------------------------
    def fix_combined(self) -> dict:
        """TODO 3: compose both levers (offload first, then retrieve)."""
