...
python run_live_bench.py <repo> --arms available,forced --reps 3 --mcp-config mcp.json
python init_claude_mds.py <repo> --prefix spring-                  # ~$0.15/module, one-time
python run_live_bench.py <repo> --arms init --reps 3
python init_claude_mds.py <repo> --remove
python aggregate.py runs
```
Understanding WHY your numbers differ from the mechanical floor is the graduation exercise.

**A reference live campaign** (Sonnet on all arms, this exact spring snapshot, **medians of 3 independent fresh sessions per cell**; raw data in `expected_output/live-run-multi.json`, three arms): structural totals — baseline **240K** tokens, graph-available **570K** (the model queries the graph, distrusts the fan-out, re-verifies against files — paying for both paths), graph-**forced** **178K** (−26%; the callers question answered in ONE turn at 42K). Forcing the graph — MCP tools as the only tools, the Graphify-style integration — is what unlocks the structural win; optional availability actively hurts with a fan-out-heavy graph. Logic questions invert it: the graph-only agent answered both "why" questions CORRECTLY by walking callee edges (logic totals: baseline 345K / graph-available 389K / graph-forced **758K**, +120%) — the graph can substitute for reading code, expensively, but can't beat it at comprehension (and on a codebase outside the model's training data, graph-shaped confidence over unread code is a hallucination risk). A fourth arm — **init** (headless CLAUDE.md in all 22 modules, then baseline tools) — measured the ADD lever: +2% overall, a wash that hides texture (−30% on one navigation question where the CLAUDE.md map sent the agent straight to the right module in 2 turns; +58% on another where the loaded context never paid off). CLAUDE.mds are orientation context, not a lookup index. The synthesis is M10's router: route structural lookups to the forced graph, comprehension to files — ~523K for these four questions vs 585K all-baseline. Full-grid medians: baseline 585K / init 600K / available 959K / forced 936K.

**The cautionary tale is part of the lab**: our first two campaigns reported "graph lost +70%" on numbers that were INVALID — the MCP server was silently crashing at startup in every session (SDK-2.x import rename), and with file tools as fallback the failure was invisible. Only the forced arm, which had no fallback to hide behind, exposed it. Verify the server is actually serving (Phase 3's smoke test) before trusting any live measurement — the forced arm doubles as a canary. Why live numbers sit far below the harness's 33.7x: (1) a real agent's Grep returns matching *lines*, not whole files, so the baseline never pays the whole-file confirmation floor; (2) this course's bare-name server sends bloated fan-out answers (~11K tokens for `registerBeanDefinition`) where type-resolved tools send compact ones; (3) when file tools are also available, the model cross-verifies graph claims, paying for both paths. Three quantities, all correct, measuring different things: the mechanical floor (33.7x), replicated session ranges (6.8x–49x), and YOUR stack live — which here ranges from a 26% win (forced, structural) to a 120% loss (forced, logic) depending on integration mode and question class. Only a measurement on your setup, in your integration mode, describes your setup — that's why you run it.
