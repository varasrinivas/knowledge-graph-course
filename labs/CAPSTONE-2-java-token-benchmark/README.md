...
**A reference live campaign** (2 structural + 2 logic questions, Sonnet both arms, this exact spring snapshot, **medians of 3 independent fresh sessions per cell** — 24 sessions, raw data in `expected_output/live-run-summary.json` (first single run) and `live-run-multi.json` (full campaign)): the graph arm **lost** — 1,004K tokens / 26 turns vs the baseline's 585K / 15 turns at the medians (+71% overall; +119% structural, +38% logic), with comparable answer quality. Identical fresh sessions varied by up to 2x on the same question, which is why medians — not single runs — are the reportable number. Why the sign flipped vs the harness's 33.7x: (1) a real agent's Grep returns matching *lines*, not whole files, so the baseline never pays the whole-file confirmation floor; (2) this course's bare-name server sends bloated fan-out answers (~11K tokens for `registerBeanDefinition`) where type-resolved tools send compact ones; (3) the model cross-verified graph claims against files, paying for both paths; (4) logic questions were a wash (+2%) — reasoning tokens don't shrink. Three quantities, all correct, measuring different things: the mechanical floor (33.7x), replicated session ranges (6.8x–49x), and YOUR stack live (here: 0.6x). Only the third describes your setup — that's why you run it.
